...
class PurePursuit(Node):
    ...
    def closest_point_on_trajectory(self):
        ...
        if self.current_pose is not None and self.points is not None:
            ...
            car_pos = self.current_pose

            differences = self.points - self.current_pose
            relative_points = np.array([np.matmul(i,R) for i in differences]) #in car frame
            ...
            for i in range(len(relative_points) - 1):
                ...
                if xdot[i]> 0: #in front of car
                    distance = self.minimum_distance(segment_start, segment_end, np.array([0,0,0]))
                    if distance < min_distance_squared:
                        ...
                        index = i
                # else:
                #     self.get_logger().info("behind")
            
            distance_to_goal = self.distance(np.array([0,0,0]), relative_points[-1])

            if closest_point is None: #no points in front of car
                # self.get_logger().info("no points in front of car")
                return True, None, None, None   
            
            if distance_to_goal < 0.001: #close enough to goal
                # self.get_logger().info("close enough to goal")
                return True, None, None, None
            
            markers = []
            count = 0
            for p in closest_segment:
                global_p = np.matmul(p, np.linalg.inv(R)) + car_pos

                marker = self.to_marker(global_p,count)
            ...
            self.relative_point_pub.publish(markerarray)

            return closest_point, closest_segment, index, distance_to_goal 
    ...
    def pose_callback(self, odometry_msg):
        ...
        if self.points is not None:
            ...
            closest_point, closest_segment, index, distance_to_goal = self.closest_point_on_trajectory()
            ...
            if isinstance(closest_point, bool):
                ...
                while not success and i < len(relative_points) - 1:
                    ...
                    count = 0
                    for p in segment:
                        global_p = np.matmul(p, np.linalg.inv(R)) + self.current_pose
                        marker = self.to_marker(global_p,count)
                        markers.append(marker)
                        count+=1
                    markerarray = MarkerArray()
                    ...
                    success, intersect = self.circle_intersection(np.array([0,0,0]), self.lookahead, segment_start, segment_end, R)
                    i += 1
    ...
    def circle_intersection(self, center, radius, p1, p2, R):
        """
        ...
        disc = b**2 - 4 * a * c
        if disc < 0: #line misses the cirlce entirely
        ...
        if intersection_points[0][0] > 0 and intersection_points[1][0] < 0:
            ...
            self.get_logger().info("both points behind car, which is weird")
        # t = max(0, min(1, - b / (2 * a)))
        # return True, P1 + t * V
